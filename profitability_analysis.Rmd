...
p1 <- recovered_data %>% 
  ...
  annotate(geom = 'text', x = 600, y = 600, 
             label = paste("MAE:", mae), 
             color = 'blue') +
  geom_text(x=1500, y=1450, label="45º line", angle=45, size=3, aes(family="mono")) +
  theme_minimal()
...
To handle this, we generate more "synthetic" data points for the smaller cities that are similar to the actual data for those cities, but with small variations. We use a popular oversampling method called Synthetic Minority Over-sampling TEchnique - SMOTE [@SMOTE].
...
distrib_cols2 <- c("Redeveloped House Size" =  "redev_house_size",
                  ...
                  "Land Acquisition Cost Multiplier" =  "cost_land_acq_multiplier",
...
sim_distribs %>% 
  filter(variable %in% names(distrib_cols1)) %>%
  mutate(city_size = if_else(city_size == "Large", "Larger Cities", "Smaller Cities")) %>% 
  rbind(tibble(city_size = "Smaller Cities", variable = "LSI", value = 1)) %>% 
  rbind(tibble(city_size = "Smaller Cities", variable = "Households", value = 5000)) %>% 
  ggplot() +
...
rbind(get_sensitivity(small, "sale", "small", floor_area_ratio, seq(2, 6)),
  ...
  mutate(`City Size` = if_else(city %in% large_city_list, "Larger", "Smaller")) %>% 
